...
// is the best way to enable modern POSIX APIs, such as posix_madvise(), on
// Solaris. (see also
...
namespace internal {
    ...
    namespace {
        ...
        ReplaceChars(std::basic_string<CharT> s, CharT find, CharT rep) {
            if (find != rep) {
        ...
        Result<NativePathString>
        StringToNative(std::string_view s) {
#if _WIN32
...
#if _WIN32
        Result<std::string>
        NativeToString(const NativePathString& ws) {
        ...
        Status
        ValidatePath(std::string_view s) {
    ...
    std::string
    ErrnoMessage(int errnum) {
        return std::strerror(errnum);
    }

#if _WIN32
    std::string
    WinErrorMessage(int errnum) {
        constexpr DWORD max_n_chars = 1024;
        WCHAR utf16_message[max_n_chars];
        auto n_utf16_chars = FormatMessageW(
        ...
        auto utf8_message_result = arrow::util::WideStringToUTF8(
            std::wstring(utf16_message, n_utf16_chars));
        if (!utf8_message_result.ok()) {
            std::stringstream ss;
            ss << "Windows error #" << errnum;
            ss << "; failed to convert error message to UTF-8: "
               << utf8_message_result.status();
            return ss.str();
        }
    ...
    namespace {
        ...
        public:
            explicit ErrnoDetail(int errnum)
                : errnum_(errnum) {}

            const char*
            type_id() const override {
                return kErrnoDetailTypeId;
            }
        ...
        public:
            explicit WinErrorDetail(int errnum)
                : errnum_(errnum) {}

            const char*
            type_id() const override {
                return kWinErrorDetailTypeId;
            }

            std::string
            ToString() const override {
                std::stringstream ss;
                ss << "[Windows error " << errnum_ << "] "
                   << WinErrorMessage(errnum_);
                return ss.str();
            }
            ...
            int
            equivalent_errno() const {
                return WinErrorToErrno(errnum_);
            }
        ...
        public:
            explicit SignalDetail(int signum)
                : signum_(signum) {}

            const char*
            type_id() const override {
                return kSignalDetailTypeId;
            }
    ...
    std::shared_ptr<StatusDetail>
    StatusDetailFromErrno(int errnum) {
        if (!errnum) {
            return nullptr;
        }
...
#if _WIN32
    std::shared_ptr<StatusDetail>
    StatusDetailFromWinError(int errnum) {
        if (!errnum) {
            return nullptr;
        }
    ...
    std::shared_ptr<StatusDetail>
    StatusDetailFromSignal(int signum) {
        return std::make_shared<SignalDetail>(signum);
    }

    int
    ErrnoFromStatus(const Status& status) {
        ...
        if (detail != nullptr) {
            ...
            if (detail->type_id() == kWinErrorDetailTypeId) {
                return checked_cast<const WinErrorDetail&>(*detail)
                    .equivalent_errno();
            }
#endif
        }
    ...
    int
    WinErrorFromStatus(const Status& status) {
#if _WIN32
        const auto detail = status.detail();
    ...
    int
    SignalFromStatus(const Status& status) {
        const auto detail = status.detail();
    ...
    namespace {
        ...
        Result<NativePathString>
        NativeReal(const NativePathString& path) {
#if _WIN32
            std::array<wchar_t, _MAX_PATH> resolved = {};
            if (_wfullpath(const_cast<wchar_t*>(path.c_str()), resolved.data(),
                    resolved.size())
                == nullptr) {
                return IOErrorFromWinError(
                    errno, "Failed to resolve real path");
            }
#else
    ...
    struct PlatformFilename::Impl {
        Impl() = default;
        explicit Impl(NativePathString p)
            : native_(NativeSlashes(std::move(p))) {}
    ...
    PlatformFilename::PlatformFilename(Impl impl)
    ...
    PlatformFilename&
    PlatformFilename::operator=(const PlatformFilename& other) {
    ...
    bool
    PlatformFilename::operator==(const PlatformFilename& other) const {
    ...
    const NativePathString&
    PlatformFilename::ToNative() const {
        return impl_->native_;
    }

    std::string
    PlatformFilename::ToString() const {
#if _WIN32
    ...
    Result<PlatformFilename>
    PlatformFilename::Real() const {
        ARROW_ASSIGN_OR_RAISE(auto real, NativeReal(ToNative()));
        return PlatformFilename(std::move(real));
    }

    Result<PlatformFilename>
    PlatformFilename::FromString(std::string_view file_name) {
    ...
    PlatformFilename
    PlatformFilename::Join(const PlatformFilename& child) const {
        if (impl_->native_.empty() || impl_->native_.back() == kNativeSep) {
            return PlatformFilename(
                Impl{impl_->native_ + child.impl_->native_});
        } else {
            return PlatformFilename(
                Impl{impl_->native_ + kNativeSep + child.impl_->native_});
        }
    }

    Result<PlatformFilename>
    PlatformFilename::Join(std::string_view child_name) const {
        ARROW_ASSIGN_OR_RAISE(
            auto child, PlatformFilename::FromString(std::string(child_name)));
        return Join(child);
    }
    ...
    namespace {
        ...
        Result<bool>
        DoCreateDir(const PlatformFilename& dir_path, bool create_parents) {
#ifdef _WIN32
            ...
            if (errnum == ERROR_ALREADY_EXISTS) {
                ...
                if (attrs == INVALID_FILE_ATTRIBUTES
                    ...
                    // Note we propagate the original error, not the
                    // GetFileAttributesW() error
                    return IOErrorFromWinError(ERROR_ALREADY_EXISTS,
                        "Cannot create directory '", dir_path.ToString(),
            ...
            return IOErrorFromWinError(GetLastError(),
                "Cannot create directory '", dir_path.ToString(), "'");
#else
            const auto s = dir_path.ToNative().c_str();
            ...
            if (errno == EEXIST) {
                ...
                if (stat(s, &st) || !S_ISDIR(st.st_mode)) {
                    // Note we propagate the original errno, not the stat()
                    // errno
                    return IOErrorFromErrno(EEXIST, "Cannot create directory '",
            ...
            return IOErrorFromErrno(
                errno, "Cannot create directory '", dir_path.ToString(), "'");
#endif
        }
    ...
    Result<bool>
    CreateDir(const PlatformFilename& dir_path) {
        return DoCreateDir(dir_path, false);
    }

    Result<bool>
    CreateDirTree(const PlatformFilename& dir_path) {
    ...
    namespace {
        ...
        void
        FindHandleDeleter(HANDLE* handle) {
            if (!FindClose(*handle)) {
        ...
        std::wstring
        PathWithoutTrailingSlash(const PlatformFilename& fn) {
        ...
        Result<std::vector<WIN32_FIND_DATAW>>
        ListDirInternal(const PlatformFilename& dir_path) {
            WIN32_FIND_DATAW find_data;
            std::wstring pattern
                = PathWithoutTrailingSlash(dir_path) + L"\\*.*";
            HANDLE handle = FindFirstFileW(pattern.c_str(), &find_data);
            if (handle == INVALID_HANDLE_VALUE) {
                return IOErrorFromWinError(GetLastError(),
                    "Cannot list directory '", dir_path.ToString(), "'");
            }

            std::unique_ptr<HANDLE, decltype(&FindHandleDeleter)> handle_guard(
                &handle, FindHandleDeleter);
        ...
        Status
        FindOneFile(const PlatformFilename& fn, WIN32_FIND_DATAW* find_data,
            bool* exists = nullptr) {
            HANDLE handle = FindFirstFileW(
                PathWithoutTrailingSlash(fn).c_str(), find_data);
            if (handle == INVALID_HANDLE_VALUE) {
                int errnum = GetLastError();
                if (exists == nullptr
                    || (errnum != ERROR_PATH_NOT_FOUND
                        && errnum != ERROR_FILE_NOT_FOUND)) {
                    return IOErrorFromWinError(GetLastError(),
                        "Cannot get information for path '", fn.ToString(),
                        "'");
                }
                *exists = false;
            } else {
    ...
    Result<std::vector<PlatformFilename>>
    ListDir(const PlatformFilename& dir_path) {
    ...
    ListDir(const PlatformFilename& dir_path) {
        ...
        if (dir == nullptr) {
            return IOErrorFromErrno(
                errno, "Cannot list directory '", dir_path.ToString(), "'");
        }

        auto dir_deleter = [](DIR* dir) -> void {
            if (closedir(dir) != 0) {
                ARROW_LOG(WARNING)
                    << "Cannot close directory handle: " << ErrnoMessage(errno);
            }
        };
    ...
    namespace {
        ...
        Status
        DeleteDirEntryDir(const PlatformFilename& path,
            const WIN32_FIND_DATAW& entry, bool remove_top_dir = true) {
            ...
            if (remove_top_dir) {
                // Remove now empty directory or reparse point (e.g. symlink to
                // dir)
                if (!RemoveDirectoryW(path.ToNative().c_str())) {
                    return IOErrorFromWinError(GetLastError(),
                        "Cannot delete directory entry '", path.ToString(),
                        "': ");
                }
            }
            return Status::OK();
        ...
        Status
        DeleteDirEntry(
            const PlatformFilename& path, const WIN32_FIND_DATAW& entry) {
            ...
            if (!DeleteFileW(path.ToNative().c_str())) {
                return IOErrorFromWinError(GetLastError(),
                    "Cannot delete file '", path.ToString(), "': ");
            }
            return Status::OK();
        }

        Status
        DeleteDirTreeInternal(const PlatformFilename& dir_path) {
            ARROW_ASSIGN_OR_RAISE(auto entries, ListDirInternal(dir_path));
            for (const auto& entry : entries) {
                PlatformFilename path
                    = dir_path.Join(PlatformFilename(entry.cFileName));
        ...
        Result<bool>
        DeleteDirContents(const PlatformFilename& dir_path,
            bool allow_not_found, bool remove_top_dir) {
            bool exists = true;
            ...
            if (exists) {
                RETURN_NOT_OK(
                    DeleteDirEntryDir(dir_path, entry, remove_top_dir));
            }
            return exists;
        }
        ...
        Status
        LinkStat(const PlatformFilename& path, struct stat* lst,
            bool* exists = nullptr) {
            if (lstat(path.ToNative().c_str(), lst) != 0) {
                if (exists == nullptr
                    || (errno != ENOENT && errno != ENOTDIR
                        && errno != ELOOP)) {
                    return IOErrorFromErrno(errno,
                        "Cannot get information for path '", path.ToString(),
                        "'");
                }
                *exists = false;
        ...
        Status
        DeleteDirEntryDir(const PlatformFilename& path, const struct stat& lst,
        ...
        Status
        DeleteDirEntry(const PlatformFilename& path, const struct stat& lst) {
        ...
        DeleteDirContents(const PlatformFilename& dir_path,
            ...
            if (exists) {
                if (!S_ISDIR(lst.st_mode) && !S_ISLNK(lst.st_mode)) {
                    return Status::IOError("Cannot delete directory '",
                        dir_path.ToString(), "': not a directory");
                }
    ...
    Result<bool>
    DeleteDirContents(const PlatformFilename& dir_path, bool allow_not_found) {
        return DeleteDirContents(
            dir_path, allow_not_found, /*remove_top_dir=*/false);
    }

    Result<bool>
    DeleteDirTree(const PlatformFilename& dir_path, bool allow_not_found) {
        return DeleteDirContents(
            dir_path, allow_not_found, /*remove_top_dir=*/true);
    }

    Result<bool>
    DeleteFile(const PlatformFilename& file_path, bool allow_not_found) {
#ifdef _WIN32
        if (DeleteFileW(file_path.ToNative().c_str())) {
            ...
            if (!allow_not_found || errnum != ERROR_FILE_NOT_FOUND) {
                return IOErrorFromWinError(GetLastError(),
                    "Cannot delete file '", file_path.ToString(), "'");
            }
        }
#else
        if (unlink(file_path.ToNative().c_str()) == 0) {
            ...
            if (!allow_not_found || errno != ENOENT) {
                return IOErrorFromErrno(
                    errno, "Cannot delete file '", file_path.ToString(), "'");
            }
        }
#endif
        return false;
    ...
    Result<bool>
    FileExists(const PlatformFilename& path) {
#ifdef _WIN32
        if (GetFileAttributesW(path.ToNative().c_str())
            != INVALID_FILE_ATTRIBUTES) {
            return true;
        } else {
            int errnum = GetLastError();
            if (errnum != ERROR_PATH_NOT_FOUND
                && errnum != ERROR_FILE_NOT_FOUND) {
                return IOErrorFromWinError(GetLastError(),
                    "Failed getting information for path '", path.ToString(),
                    "'");
            }
            return false;
        }
    ...
    void
    FileDescriptor::CloseFromDestructor(int fd) {
    ...
    lseek64_compat(int fd, int64_t pos, int whence) {
#if defined(_WIN32)
    ...
    FileOpenReadable(const PlatformFilename& file_name) {
        FileDescriptor fd;
#if defined(_WIN32)
        HANDLE file_handle = CreateFileW(file_name.ToNative().c_str(),
            GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (file_handle == INVALID_HANDLE_VALUE) {
            return IOErrorFromWinError(GetLastError(),
                "Failed to open local file '", file_name.ToString(), "'");
        }
        ...
        if (ret == 0 && S_ISDIR(st.st_mode)) {
            return Status::IOError("Cannot open for reading: path '",
                file_name.ToString(), "' is a directory");
        }
#endif
    ...
    Result<FileDescriptor>
    FileOpenWritable(const PlatformFilename& file_name, bool write_only,
    ...
    CreatePipe() {
        ...
        // On Unix, we don't want the file descriptors to survive after an
        // exec() call
...
#define PIPE_READ read
        ...
        class SelfPipeImpl : public SelfPipe,
                             public std::enable_shared_from_this<SelfPipeImpl> {
            static constexpr uint64_t kEofPayload = 5804561806345822987ULL;

        public:
            explicit SelfPipeImpl(bool signal_safe)
                : signal_safe_(signal_safe) {}
            ...
            Init() {
                ...
                if (signal_safe_) {
                    if (!please_shutdown_.is_lock_free()) {
                        return Status::IOError("Cannot use non-lock-free "
                                               "atomic in a signal handler");
                    }
                    // We cannot afford blocking writes in a signal handler
                    RETURN_NOT_OK(
                        SetPipeFileDescriptorNonBlocking(pipe_.wfd.fd()));
                }

                atfork_handler_ = std::make_shared<AtForkHandler>(
                    /*before=*/
                    [weak_self
                        = std::weak_ptr<SelfPipeImpl>(shared_from_this())] {
                    ...
                    [](std::any token) {
                        auto self
                            = std::any_cast<std::shared_ptr<SelfPipeImpl>>(
                                std::move(token));
            ...
            Wait() override {
                ...
                while (buf_size > 0) {
                    int64_t n_read = PIPE_READ(
                        pipe_.rfd.fd(), buf, static_cast<uint32_t>(buf_size));
                    if (n_read < 0) {
                        if (errno == EINTR) {
                            continue;
                        }
                        ...
                        return IOErrorFromErrno(
                            errno, "Failed reading from self-pipe");
                    }
                    buf += n_read;
                    buf_size -= n_read;
                }
            ...
            void
            Send(uint64_t payload) override {
                if (signal_safe_) {
            ...
            Shutdown() override {
                ...
                if (!DoSend(kEofPayload)) {
                    if (errno) {
                        return IOErrorFromErrno(
                            errno, "Could not shutdown self-pipe");
                    } else if (!pipe_.wfd.closed()) {
                        return Status::UnknownError(
                            "Could not shutdown self-pipe");
                    }
                }
            ...
            ~SelfPipeImpl() {
                ARROW_WARN_NOT_OK(Shutdown(), "On self-pipe destruction");
            }
        ...
        protected:
            ...
            ChildAfterFork() {
                ...
                const bool was_closed
                    = pipe_.rfd.closed() || pipe_.wfd.closed();
            ...
            Status
            ClosedPipe() const {
                return Status::Invalid("Self-pipe closed");
            }

            bool
            DoSend(uint64_t payload) {
                ...
                auto buf_size = static_cast<int64_t>(sizeof(payload));
                while (buf_size > 0) {
                    int64_t n_written = PIPE_WRITE(
                        pipe_.wfd.fd(), buf, static_cast<uint32_t>(buf_size));
                    if (n_written < 0) {
                        if (errno == EINTR) {
                            continue;
                        } else {
                            // Perhaps EAGAIN if non-blocking, or EBADF if
                            // closed in the meantime? In any case, we can't do
                            // anything more here.
                            break;
                        }
                    }
...
#undef PIPE_READ
    ...
    Result<std::shared_ptr<SelfPipe>>
    SelfPipe::Make(bool signal_safe) {
    ...
    namespace {
        ...
        Status
        StatusFromMmapErrno(const char* prefix) {
#ifdef _WIN32
        ...
        GetPageSizeInternal() {
            ...
            if (ret == -1) {
                ARROW_LOG(FATAL)
                    << "sysconf(_SC_PAGESIZE) failed: " << ErrnoMessage(errno);
            }
    ...
    Status
    MemoryMapRemap(void* addr, size_t old_size, size_t new_size, int fildes,
        void** new_addr) {
        ...
        *new_addr = MAP_FAILED;
        ...
        LONG new_size_high
            = static_cast<LONG>((new_size64 >> 32) & 0xFFFFFFFFUL);
        ...
        *new_addr = mmap(
            NULL, new_size, PROT_READ | PROT_WRITE, MAP_SHARED, fildes, 0);
        if (*new_addr == MAP_FAILED) {
    ...
    Status
    MemoryAdviseWillNeed(const std::vector<MemoryRegion>& regions) {
        ...
        struct PrefetchEntry { // Like WIN32_MEMORY_RANGE_ENTRY
            ...
            PrefetchEntry(const MemoryRegion& region) // NOLINT runtime/explicit
                : VirtualAddress(region.addr)
                , NumberOfBytes(region.size) {}
        };
        using PrefetchVirtualMemoryFunc
            = BOOL (*)(HANDLE, ULONG_PTR, PrefetchEntry*, ULONG);
        static const auto prefetch_virtual_memory
            = reinterpret_cast<PrefetchVirtualMemoryFunc>(GetProcAddress(
        ...
        if (prefetch_virtual_memory != nullptr) {
            ...
            if (!entries.empty()
                && !prefetch_virtual_memory(GetCurrentProcess(),
                    ...
                    0)) {
                return IOErrorFromWinError(
                    GetLastError(), "PrefetchVirtualMemory failed");
            }
        }
        return Status::OK();
        ...
        for (const auto& region : regions) {
            if (region.size != 0) {
                ...
                int err = posix_madvise(
                    aligned.addr, aligned.size, POSIX_MADV_WILLNEED);
                // EBADF can be returned on Linux in the following cases:
                // - the kernel version is older than 3.9
                // - the kernel was compiled with CONFIG_SWAP disabled
                // (ARROW-9577)
                if (err != 0 && err != EBADF) {
    ...
    Status
    FileSeek(int fd, int64_t pos, int whence) {
        return lseek64_compat(fd, pos, whence).status();
    }

    Status
    FileSeek(int fd, int64_t pos) {
        return FileSeek(fd, pos, SEEK_SET);
    }
    ...
    pread_compat(int fd, void* buf, int64_t nbytes, int64_t pos) {
#if defined(_WIN32)
        ...
        BOOL bRet = ReadFile(handle, buf, static_cast<uint32_t>(nbytes),
        ...
        int64_t ret;
        do {
            ret = static_cast<int64_t>(pread(
                fd, buf, static_cast<size_t>(nbytes), static_cast<off_t>(pos)));
    ...
    Result<int64_t>
    FileRead(int fd, uint8_t* buffer, int64_t nbytes) {
#if defined(_WIN32)
        HANDLE handle = reinterpret_cast<HANDLE>(_get_osfhandle(fd));
#endif
        ...
        while (total_bytes_read < nbytes) {
            const int64_t chunksize
                = std::min(static_cast<int64_t>(ARROW_MAX_IO_CHUNKSIZE),
            ...
            if (!ReadFile(handle, buffer, static_cast<uint32_t>(chunksize),
                ...
                if (errnum != ERROR_HANDLE_EOF && errnum != ERROR_BROKEN_PIPE) {
                    return IOErrorFromWinError(
                        GetLastError(), "Error reading bytes from file");
                }
            }
            bytes_read = dwBytesRead;
#else
            bytes_read = static_cast<int64_t>(
                read(fd, buffer, static_cast<size_t>(chunksize)));
            if (bytes_read == -1) {
                if (errno == EINTR) {
                    continue;
                }
    ...
    Result<int64_t>
    FileReadAt(int fd, uint8_t* buffer, int64_t position, int64_t nbytes) {
        int64_t bytes_read = 0;
    ...
    Status
    FileWrite(int fd, const uint8_t* buffer, const int64_t nbytes) {
...
#if defined(_WIN32)
            int64_t ret = static_cast<int64_t>(_write(
                fd, buffer + bytes_written, static_cast<uint32_t>(chunksize)));
#else
            int64_t ret = static_cast<int64_t>(write(
                fd, buffer + bytes_written, static_cast<size_t>(chunksize)));
    ...
    Status
    FileTruncate(int fd, const int64_t size) {
        ...
        if (ret == -1) {
            return IOErrorFromErrno(
                errno_actual, "Error writing bytes to file");
        }
        return Status::OK();
    }
...
#ifdef _WIN32
    Result<NativePathString>
    GetEnvVarNative(const std::string& name) {
    ...
    Result<NativePathString>
    GetEnvVarNative(const char* name) {
    ...
    Status
    DelEnvVar(const std::string& name) {
        return DelEnvVar(name.c_str());
    }
    ...
    namespace {
        ...
        std::vector<NativePathString>
        GetPlatformTemporaryDirs() {
...
#if _WIN32
            selectors = {{"TMP", L""}, {"TEMP", L""}, {"LOCALAPPDATA", L"Temp"},
                {"USERPROFILE", L"Temp"}};
            ...
            for (const auto& sel : selectors) {
                ...
                if (!result.ok()) {
                    ARROW_LOG(WARNING)
                        << "Failed getting env var '" << sel.env_var
    ...
    TemporaryDir::Make(const std::string& prefix) {
        ...
        auto TryCreatingDirectory = [&](const NativePathString& base_dir)
            -> Result<std::unique_ptr<TemporaryDir>> {
            Status st;
            for (int attempt = 0; attempt < 3; ++attempt) {
                ...
                if (*result) {
                    return std::unique_ptr<TemporaryDir>(
                        new TemporaryDir(std::move(fn)));
                }
                // The random name already exists in base_dir, try with another
                // name
                st = Status::IOError(
                    "Path already exists: '", fn.ToString(), "'");
        ...
        return Status::IOError("Cannot create temporary subdirectory in any "
                               "of the platform temporary directories");
    }

    TemporaryDir::TemporaryDir(PlatformFilename&& path)
        : path_(std::move(path)) {}
    ...
    SignalHandler::SignalHandler()
        : SignalHandler(static_cast<Callback>(nullptr)) {}
    ...
    SignalHandler::action() const {
        return sa_;
    }
#endif

    Result<SignalHandler>
    GetSignalHandler(int signum) {
#if ARROW_HAVE_SIGACTION
        struct sigaction sa;
...
#else
        // To read the old handler, set the signal handler to something else
        // temporarily
    ...
    Result<SignalHandler>
    SetSignalHandler(int signum, const SignalHandler& handler) {
#if ARROW_HAVE_SIGACTION
    ...
    void
    ReinstateSignalHandler(int signum, SignalHandler::Callback handler) {
#if !ARROW_HAVE_SIGACTION
    ...
    Status
    SendSignalToThread(int signum, uint64_t thread_id) {
#ifdef _WIN32
        return Status::NotImplemented(
            "Cannot send signal to specific thread on Windows");
#else
        // Have to use a C-style cast because pthread_t can be a pointer *or*
        // integer type
        // int r = pthread_kill(
        //     (pthread_t)thread_id, signum); // NOLINT readability-casting
        // if (r == 0) {
        //     return Status::OK();
        // }
        // if (r == EINVAL) {
        //     return Status::Invalid("Invalid signal number ", signum);
        // }
        return IOErrorFromErrno(EINVAL, "Failed to raise signal");
#endif
    }
    ...
    namespace {
        ...
        GetSeedGenerator() {
            ...
            // Make sure to mix in process id to minimize risks of clashes when
            // parallel testing.
#ifdef ARROW_VALGRIND
            // Valgrind can crash, hang or enter an infinite loop on
            // std::random_device, use a crude initializer instead.
    ...
    int64_t
    GetRandomSeed() {
        // The process-global seed generator to aims to avoid calling
        // std::random_device unless truly necessary (it can block on some
        // systems, see ARROW-10287).
    ...
    GetCurrentRSS() {
        ...
        if (fp) {
            ...
            ARROW_LOG(WARNING)
                << "Can't resolve RSS value from /proc/self/statm";
            return 0;
        }
    ...
    GetTotalMemoryBytes() {
        ...
        if (sysinfo(&info) == -1) {
            ARROW_LOG(WARNING)
                << "Failed to resolve total RAM size: " << std::strerror(errno);
            return -1;
        }
