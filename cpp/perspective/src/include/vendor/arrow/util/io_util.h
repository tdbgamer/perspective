...
namespace internal {
    ...
    /// Delete a directory's contents (but not the directory itself) if it
    /// exists.
    ///
    /// Return whether the directory existed.
    ARROW_EXPORT
    Result<bool> DeleteDirContents(
        const PlatformFilename& dir_path, bool allow_not_found = true);

    /// Delete a directory tree if it exists.
    ///
    /// Return whether the directory existed.
    ARROW_EXPORT
    Result<bool> DeleteDirTree(
        const PlatformFilename& dir_path, bool allow_not_found = true);
    ...
    ARROW_EXPORT
    Result<std::vector<PlatformFilename>> ListDir(
        const PlatformFilename& dir_path);
    ...
    Result<bool> DeleteFile(
        const PlatformFilename& file_path, bool allow_not_found = true);
    ...
    Result<FileDescriptor> FileOpenWritable(const PlatformFilename& file_name,
        bool write_only = true, bool truncate = true, bool append = false);
    ...
    ARROW_EXPORT
    Result<int64_t> FileReadAt(
        int fd, uint8_t* buffer, int64_t position, int64_t nbytes);
    ...
    ARROW_EXPORT
    Status MemoryMapRemap(void* addr, size_t old_size, size_t new_size,
        int fildes, void** new_addr);
    ARROW_EXPORT
    ...
    Status
    StatusFromErrno(int errnum, StatusCode code, Args&&... args) {
        return Status::FromDetailAndArgs(
            code, StatusDetailFromErrno(errnum), std::forward<Args>(args)...);
    }

    template <typename... Args>
    Status
    IOErrorFromErrno(int errnum, Args&&... args) {
        return StatusFromErrno(
            errnum, StatusCode::IOError, std::forward<Args>(args)...);
    }

#if _WIN32
    template <typename... Args>
    Status
    StatusFromWinError(int errnum, StatusCode code, Args&&... args) {
    ...
    template <typename... Args>
    Status
    IOErrorFromWinError(int errnum, Args&&... args) {
        return StatusFromWinError(
            errnum, StatusCode::IOError, std::forward<Args>(args)...);
    }
#endif
    ...
    template <typename... Args>
    Status
    StatusFromSignal(int signum, StatusCode code, Args&&... args) {
        return Status::FromDetailAndArgs(
            code, StatusDetailFromSignal(signum), std::forward<Args>(args)...);
    }

    template <typename... Args>
    Status
    CancelledFromSignal(int signum, Args&&... args) {
        return StatusFromSignal(
            signum, StatusCode::Cancelled, std::forward<Args>(args)...);
    }
    ...
    public:
        ...
        static Result<std::unique_ptr<TemporaryDir>> Make(
    ...
    protected:
#if ARROW_HAVE_SIGACTION
        // Storing the full sigaction allows to restore the entire signal
    ...
    Result<SignalHandler> SetSignalHandler(
        int signum, const SignalHandler& handler);

    /// \brief Reinstate the signal handler
    ///
    /// For use in signal handlers.  This is needed on platforms without
    /// sigaction() such as Windows, as the default signal handler is restored
    /// there as soon as a signal is raised.
    ARROW_EXPORT
    ...
    ///
    /// This function may be slightly costly, so should only be used to
    /// initialize a PRNG, not to generate a large amount of random numbers. It
    /// is better to use this function rather than std::random_device, unless
    ...
    /// value is a regular integer value, which is more convenient than an
    /// opaque type.
    ARROW_EXPORT
    ...
    ///
    /// This function supports Windows, Linux, and Mac and will return 0
    /// otherwise
    ARROW_EXPORT
